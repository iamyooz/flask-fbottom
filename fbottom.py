...
kmeans_model = joblib.load('fbottom_kmeans_model.pkl')  # Changed from ctop_kmeans_model.pkl
scaler = None
learning_data = None
...
def load_scaler_and_data():
    ...
    if scaler is None:
        with open('formal_bottom_cluster.json', 'r') as json_file:  # Changed from casual_top_cluster.json
...
@app.route('/fbottom', methods=['POST'])
